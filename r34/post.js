...
if (new URLSearchParams(location.search).get('format') === 'json') {
	url = getLink(window.location.hash);
	fetchData(url);
	artists = getTagOfType(post.tags, "artist");
	const embed = {
		title: "Generated by JS!",
		author: { name: "No backend needed!" }
	};
...
function setButtons() {
	...
	function copyMedia() {
		element = document.getElementById("copyMedia").firstElementChild;
		let copy =
